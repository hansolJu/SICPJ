...
int sizeCheck(int i) {
	if (strcmp(line[i].memory, "RESW") == 0) {                   //원시문 메모리값이 RESW이면 RESW의 값만큼 WORD_SIZE 곱해줌
		...
		for (j = 0; j < strlen(line[i].location); j++) {  		//원시문 location길이만큼 포문을 돔
																...
																//break;
			if (line[i].location[j] != 'C' && line[i].location[j] != '\'') {
				count++;
			}
...
int getOpcode(int index) {
	for (int i = 0; i < MAX; i++) {
		if (strcmp(table[i].memory, line[index].memory) == 0)
			return i;
	}
}
int getLocation(char symbol[]) {
	for (int i = 0; i < MAX; i++) {
		if (strcmp(symbol, label[i].label) == 0)
			return label[i].location;
	}
}
void path2(int lines, int totallines) {
	int i = 0;
	int hex = atoi(line[0].location);
	int decimal = 0;
	int radix = 1;
	int decimalcount = 0;
	while (hex != 0) {                                   //10진수 16진수로 바꾸는데 사용
		decimal += (hex % 10) * radix;
		hex /= 10;
		radix *= 16;
	}
	int startLocation = decimal;

	printf("\n");
	printf("H %s %06x %06x\n", line[0].label, decimal, totallines - decimal);    //Header 레코드 출력
	for (i = 1; i < lines; i++) {                                              //Text 레코드 길이를 구하기 위한 포문
		if (strcmp(line[i].memory, "RESB") == 0 || strcmp(line[i].memory, "RESW") == 0)
			break;
		else if (strcmp(line[i].memory, "BYTE") == 0 && strcmp(line[i].location, "C'EOF'") == 0 && decimalcount < 30)
			decimalcount += 3;
		else if (strcmp(line[i].memory, "WORD") == 0 && decimalcount < 30)
			decimalcount += 3;
		else
			decimalcount += 3;
		if (decimalcount == 30)
	...
	printf("T %06x %02x ", decimal, decimalcount);      //Text 레코드 시작주소 길이 출력
	decimal = 0;
	decimalcount = 0;
	for (i; i <= lines; i++) {                                                  //Text 레코드 길이를 구하기 위한 포문
		if (strcmp(line[i + 1].memory, "RESB") == 0 || strcmp(line[i + 1].memory, "RESW") == 0)
			break;
		else if (strcmp(line[i + 1].memory, "BYTE") == 0 && strcmp(line[i + 1].location, "C'EOF'") == 0 && decimalcount < 30)
			decimalcount += 3;
		else if (strcmp(line[i + 1].memory, "WORD") == 0 && decimalcount < 30)
			decimalcount += 3;
		else
			decimalcount += 3;
		if (decimalcount == 30)
	...
	for (i = 1; i < lines; i++) {                                             //Text 레코드 길이와 목적코드 출력 위한 포문
		if (strcmp(line[i].memory, "RESB") == 0 || strcmp(line[i].memory, "RESW") == 0)
			break;
		else if (strcmp(line[i].memory, "BYTE") == 0 && strcmp(line[i].location, "C'EOF'") == 0 && decimal<30) {
			printf("454F46 ");
			decimal += 3;
		}
		else if (strcmp(line[i].memory, "WORD") == 0 && decimal<30) {
			int wordhex = atoi(line[i].location);

			int worddecimal = 0;
			int wordradix = 1;
			while (wordhex != 0) {                                     //10진수 16진수 변환
				worddecimal += (wordhex % 10) * wordradix;

				wordhex /= 10;
				wordradix *= 16;
			}
			printf("%06x ", worddecimal);
			decimal += 3;
		}
		else if (decimal<30) {
			int opcodeIndex = getOpcode(i);                              //opcode 찾는 함수호출
			int decimalAddress = getLocation(line[i].location);          //원시문에 적힌 location값 찾는 함수 호출 ex(RETADR = 1027)

			printf("%s%x ", table[opcodeIndex].opcode, decimalAddress);   //목적코드 출력
			decimal += 3;
		}
		if (decimal >= 30) {
			printf("\n");
			printf("T %06x %02x ", label[i].location, decimalcount);      //길이가 30넘어가면 점프? 해주고 다시 Text 레코드 시작주소 길이 출력
			decimal = 0;
		}
	}
	printf("\n");
	printf("E %06x\n", startLocation);                                 //end 레코드 출력
...
int main() {
	...
	totallines = cal(lines);

	path2(lines, totallines);
